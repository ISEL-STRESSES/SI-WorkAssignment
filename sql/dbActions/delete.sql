...
DROP TRIGGER updatePlayerStats ON joga;
DROP FUNCTION updatePlayerStats();

------------------------------------------------------------------------------------------------------------------------
-- Jogo
-- 2. Automation of the game stats calculation.
DROP TRIGGER updateGameStats ON joga;
DROP FUNCTION updateGameStats();
...
BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE;
-- Utils
-- 1. Verification if a player has a badge
DROP FUNCTION has_badge(player_id INTEGER, badge_name VARCHAR, game_id ALPHANUMERIC);
